...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!consent) {
      toast.error('Please accept the DPDP privacy agreement to register');
      return;
    }
    if (!isVerified) {
    ...
    try {
      ...
      localStorage.setItem(`vs_owner_name_${fullPhone}`, fullName.trim());
      const res = await requestOtpMutation({ phone: fullPhone });
      if (res?.message) {
        toast.success(`Verification OTP code dispatched to +91 ${phone}`);
    ...
    <Card className="glass-panel border-zinc-200 dark:border-zinc-900 bg-white dark:bg-[#0c0c0f]/95 shadow-2xl p-6 w-full max-w-sm mx-auto space-y-5 text-left text-zinc-950 dark:text-white font-sans transition-colors duration-300">
      ...
      <CardHeader className="p-0 text-center space-y-1.5">
        <div className="size-12 rounded-lg bg-brand/10 border border-brand/20 text-brand flex items-center justify-center mx-auto mb-1 shadow-sm">
          <HugeiconsIcon icon={UserAdd01Icon} className="size-6 text-brand" />
        </div>
        <CardTitle className="text-xl font-black text-zinc-900 dark:text-white font-display">
          Create VaahanSafe Account
        </CardTitle>
        <CardDescription className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
          Register your vehicle safety record in 30 seconds for emergency contact dispatch.
        </CardDescription>
      </CardHeader>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] font-mono text-zinc-500 font-bold uppercase">Full Name</label>
          <div className="flex items-center gap-2 px-3 py-1 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800/80 focus-within:border-brand rounded-lg transition-all">
            <HugeiconsIcon icon={UserAdd01Icon} className="size-4 text-zinc-400 shrink-0" />
            <Input
              type="text"
        ...
        {/* DPDP Agreement Checkbox */}
        <div className="flex items-start gap-2 pt-1">
          <Checkbox
            id="signup-dpdp-consent"
            checked={consent}
            onCheckedChange={(checked) => setConsent(!!checked)}
            className="mt-0.5"
          />
          <label htmlFor="signup-dpdp-consent" className="text-[11px] text-zinc-500 dark:text-zinc-400 leading-tight cursor-pointer">
            I agree to the <Link to="/legal" className="text-brand underline font-bold">DPDP 2023 Privacy Terms</Link> for vehicle emergency contact routing.
          </label>
        </div>
        ...
        >
          {cooldown > 0 ? (
            ...
            'Dispatching registration code...'
          ) : (
            <div className="flex items-center justify-center gap-2">
              <span>Register Account & Get OTP</span>
        ...
        <div className="text-center pt-2 border-t border-zinc-100 dark:border-zinc-900">
          ...
          >
            Already registered? <span className="text-brand font-bold underline">Log in to account</span>
          </Link>
        </div>
      </form>
    </Card>
