...
export default function ForbiddenPage() {
  ...
  // Drive from left edge and stop exactly in front of the barrier in the middle
  useEffect(() => {
    let animId: number;
    let start: number | null = null;
    const duration = 2500; // 2.5s drive duration
  ...
  const svgColors = isDark
    ? {
        ...
        roadHighlight: 0.03,
        edgeLine: '#ff5a00',
        edgeLineGlow: 'rgba(255,90,0,0.25)',
    ...
    >
      ...
      <div className="w-full max-w-2xl px-6 space-y-3 z-10 pt-8 md:pt-16 flex-shrink-0">
        <div className="relative">
          <h1
            aria-hidden="true"
            className="absolute inset-0 text-[7rem] md:text-[10rem] font-black tracking-tighter leading-none text-red-500/10 select-none text-center"
            style={{ transform: 'translate(-3px, -2px)' }}
          >
            403
          </h1>
          <h1 className="relative text-[7rem] md:text-[10rem] font-black tracking-tighter leading-none text-transparent bg-gradient-to-b from-red-400 via-red-500 to-orange-600 bg-clip-text select-none drop-shadow-[0_10px_20px_rgba(239,68,68,0.2)] text-center">
            403
          </h1>
        </div>
        ...
        <h2 className="text-lg md:text-xl font-bold text-neutral-400 uppercase tracking-[0.15em] -mt-2 flex items-center justify-center gap-2">
          <HugeiconsIcon icon={LockIcon} className="size-5 text-red-500" />
          Restricted Sector
        </h2>
        <p className="text-neutral-400 dark:text-neutral-500 text-xs md:text-sm max-w-md mx-auto leading-relaxed">
          Access denied by the security engine. Your credentials are recognized, but this module remains restricted to comply with authorization protocols.
        </p>
      </div>

      {/* ─── Road Section with Stop Barriers & Stop-Animated Car ─── */}
      <div className="relative w-full flex-1 min-h-0 flex items-center justify-center overflow-visible">
        ...
        >
          ...
          <g transform="translate(565, 83) rotate(-15)">
            ...
            {/* Warning pulse glow */}
            <circle r="36" fill="rgba(239, 68, 68, 0.15)" className="animate-pulse" />
            <circle r="14" fill="#ef4444" className="animate-ping" opacity="0.3" />
            {/* Outer reflective red border triangle */}
            <polygon points="-22,12 0,-24 22,12" fill="#dc2626" stroke="#991b1b" strokeWidth="1.5" />
            {/* Inner warning orange triangle */}
            <polygon points="-17,9 0,-18 17,9" fill="#f97316" />
            {/* Bold Exclamation point */}
            <rect x="-1.5" y="-10" width="3" height="10" rx="0.8" fill="#fafafa" />
            <circle cx="0" cy="3" r="1.8" fill="#fafafa" />
          </g>
