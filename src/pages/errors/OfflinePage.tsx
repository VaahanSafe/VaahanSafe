...
import { WifiOff01Icon, RefreshIcon } from '@hugeicons/core-free-icons';
...
export default function OfflinePage() {
  ...
  // Drive from left edge and stop in the middle
  useEffect(() => {
    let animId: number;
  ...
  const handleRetry = () => {
    ...
    setTimeout(() => {
      setChecking(false);
      if (navigator.onLine) {
        toast.success('Connection restored! Re-entering cockpit...');
        window.location.href = '/dashboard';
      } else {
        toast.error('Still offline. Please check your WiFi or cellular network link.');
      }
    }, 1000);
  };
    ...
    >
      ...
      <div className="w-full max-w-2xl px-6 space-y-3 z-10 pt-8 md:pt-16 flex-shrink-0">
        <div className="relative">
          <h1
            aria-hidden="true"
            className="absolute inset-0 text-[6rem] md:text-[8rem] font-black tracking-tighter leading-none text-amber-500/10 select-none text-center"
            style={{ transform: 'translate(-3px, -2px)' }}
          >
            OFFLINE
          </h1>
          <h1 className="relative text-[6rem] md:text-[8rem] font-black tracking-tighter leading-none text-transparent bg-gradient-to-b from-amber-400 via-orange-500 to-amber-600 bg-clip-text select-none drop-shadow-[0_10px_20px_rgba(245,158,11,0.2)] text-center">
            OFFLINE
          </h1>
        </div>

        <h2 className="text-lg md:text-xl font-bold text-neutral-400 uppercase tracking-[0.15em] -mt-2 flex items-center justify-center gap-2">
          <HugeiconsIcon icon={WifiOff01Icon} className="size-5 text-amber-500 animate-pulse" />
          Signal Disrupted
        </h2>
        <p className="text-neutral-400 dark:text-neutral-500 text-xs md:text-sm max-w-md mx-auto leading-relaxed">
          Your telemetry feed was interrupted. VaahanSafe requires an active network link to securely authorize operations and dispatch alerts.
        </p>
        ...
        <div className="pt-4 flex justify-center">
          <button
            onClick={handleRetry}
            disabled={checking}
            className="group flex items-center justify-center gap-2 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 disabled:bg-orange-500/50 text-xs font-bold text-white rounded-lg transition-all shadow-lg active:scale-[0.98]"
          >
            <HugeiconsIcon icon={RefreshIcon} className={`size-4 ${checking ? 'animate-spin' : 'group-hover:rotate-45 transition-transform duration-300'}`} />
            {checking ? 'Checking Connection...' : 'Retry Connection'}
          </button>
        </div>
      </div>

      {/* ─── Road Section with Signal Loss ─── */}
      <div className="relative w-full flex-1 min-h-0 flex items-center justify-center overflow-visible">
        ...
        >
          ...
          {/* Dotted Signal Ring pulses */}
          <g transform="translate(565, 83) rotate(-15)">
            <circle r="30" stroke="#f59e0b" strokeWidth="1" strokeDasharray="4 4" className="animate-ping opacity-25" />
            <circle r="18" stroke="#f59e0b" strokeWidth="1.5" strokeDasharray="3 3" className="animate-pulse opacity-40" />
          ...
          <g transform={`translate(${carPos.x}, ${carPos.y}) rotate(${carPos.angle})`}>
            ...
            {/* Message Flag pops up when stopped */}
      ...
      {/* ─── Telemetry Retry Button ─── */}
      <div className="w-full max-w-xs px-6 z-10 pb-8 md:pb-10 flex-shrink-0 mx-auto">
        <button
          onClick={handleRetry}
          disabled={checking}
        ...
        >
          <HugeiconsIcon icon={RefreshIcon} className={`size-4 ${checking ? 'animate-spin' : ''}`} />
          {checking ? 'Checking Connection...' : 'Retry Connection'}
        </button>
      </div>
    </div>
